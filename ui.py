...
class QuizInterFace:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(150, 125, text="Hello dhruv how are you", fill=THEME_COLOR,
                                                     font=("Arial", 20, "italic"), width=280)
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)

        check_img = tkinter.PhotoImage(file="./images/true.png")
        cross_img = tkinter.PhotoImage(file="./images/false.png")
        self.check_button = tkinter.Button(image=check_img, highlightthickness=0, command=self.true_pressed)
        self.check_button.grid(row=2, column=0)

        self.cross_button = tkinter.Button(image=cross_img, highlightthickness=0, command=self.false_pressed)
